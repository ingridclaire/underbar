(function() {
  ...
  _.last = function(array, n) {
  	...
  	return n === undefined ? array[array.length-1] : array.slice(startIndex, lastIndex)
  };
  ...
  _.reduce = function(collection, iterator, accumulator) {
  	//collection is the array or object passed in
  	//iterator is a function that takes in accumulator and item
  ...
  _.memoize = function(func) {
    ...
    var result;
    return function() {
    if(!usedArguments.includes(JSON.stringify(arguments))) {
      result = func.apply(this, arguments);
      usedArguments += JSON.stringify(arguments);
    }
    return result;
    }
  };
  ...
  _.shuffle = function(array) {
    ...
    for(var i = 0; i < copy.length; i++) {
      var index = Math.floor(Math.random() * Math.floor(result.length))
      result.splice(index, 0, copy[i]);
    }
    return result;
  };
